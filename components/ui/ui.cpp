...
// 240 × 18 × 2 = 8,640 bytes allocated from heap in ui_init() (before NimBLE
// starts) so it does not count against the static DRAM budget that BLE needs.
...
void ui_init(bool display_only) {
    ...
    // Allocate the waterfall frame buffer from heap here — before NimBLE starts —
    // so the allocation succeeds while DRAM is unfragmented.  Static BSS would
    // consume 8,640 bytes from the fixed DRAM pool that BLE also draws from.
    if (!wf_fb) {
