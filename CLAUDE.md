...
## Cardputer-launcher (G0 → menu)

This firmware carries a small **G0/BOOT → launcher** hook so FT8 can run alongside WSPR and a
GPS/LoRa app in the [cardputer-launcher](https://github.com/jeremywho/cardputer-launcher) OTA
suite (a factory-slot menu that boots whichever app you pick; FT8 lives in `ota_0`).
`return_to_launcher()` (`main/main.cpp:6430`) reads **GPIO0 raw** (`gpio_get_level(0)==0`, polled
in the UI loop) — *not* `M5.BtnA`, which `beginDisplayOnly()` never wires. On press it sets the
boot partition to `factory` and resets, but **only if** `esp_ota_get_running_partition() != factory`.
Flashed standalone, FT8 *is* the factory app → the hook is a **true no-op**, so the same
`mini_ft8.bin` runs both standalone and as the suite's `ota_0` (OTA images are slot-independent).
See the launcher repo for the partition map and one-shot flashing.
